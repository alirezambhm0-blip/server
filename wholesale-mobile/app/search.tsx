...
export default function SearchScreen() {
  ...
  const executeSearch = async (q = query) => {
    ...
    try {
      ...
      setResults(res);
      if (!res.has_results || res.products.data.length === 0) {
        setActiveState('NO_RESULTS');
      }
      
      if (isLoggedIn) {
        searchApi.addHistory(q, res.total_results).catch(() => {});
      }
    } catch (e) {
      setActiveState('NO_RESULTS');
    } finally {
                ...
                {categories.map(cat => (
                  ...
                  >
                    ...
                    <View style={styles.tileContent}>
                      ...
                      <Text style={styles.tileCount}>{cat.productCount || 0} محصول</Text>
                    </View>
                  </TouchableOpacity>
                ))}
              </View>
          ...
          <View style={{ flex: 1 }}>
            <View style={styles.resultsToolbar}>
              <Text style={styles.resultsCount}>
                {results?.total_results || 0} نتیجه برای «{query}»
              </Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  tileImg: { width: 45, height: 45, borderRadius: 8 },
  tileContent: { flex: 1, marginRight: 10 },
  tileName: { fontSize: 13, fontWeight: '700', color: COLORS.textPrimary, textAlign: 'right' },
  tileCount: { fontSize: 11, color: '#94A3B8', textAlign: 'right', marginTop: 2 },
  resultsToolbar: { 
    height: 40, 
    backgroundColor: '#FFF', 
  ...
  resultsCount: { fontSize: 12, color: COLORS.textSecondary, textAlign: 'right' },
});
